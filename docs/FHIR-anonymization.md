...
### Features
* Support anonymization of FHIR R5, R4 and STU3 data in JSON as well as NDJSON format
...
This tool supports three FHIR versions with separate executables for each:

| FHIR Version | Executable | Notes |
|--------------|------------|-------|
| **R5** | Microsoft.Health.Fhir.Anonymizer.R5.CommandLineTool.exe | Latest FHIR specification (v5.0.0). Includes new resources like Subscription, SubscriptionTopic, ActorDefinition, Requirements, and ArtifactAssessment. |
| **R4** | Microsoft.Health.Fhir.Anonymizer.R4.CommandLineTool.exe | FHIR R4 specification (v4.0.1) |
| **STU3** | Microsoft.Health.Fhir.Anonymizer.Stu3.CommandLineTool.exe | FHIR STU3 specification (v3.0.2) |

**Important:** Each version has its own configuration file. Ensure your configuration file's `fhirVersion` field matches the executable you're using.
...
Once you have built the command line tool, you will find three executable files for R5, R4 and STU3 respectively: 

1. Microsoft.Health.Fhir.Anonymizer.R5.CommandLineTool.exe in the $SOURCE\FHIR\src\Microsoft.Health.Fhir.Anonymizer.R5.CommandLineTool\bin\Debug|Release\net8.0 folder.
...
3. Microsoft.Health.Fhir.Anonymizer.Stu3.CommandLineTool.exe in the $SOURCE\FHIR\src\Microsoft.Health.Fhir.Anonymizer.Stu3.CommandLineTool\bin\Debug|Release\net8.0 folder.
...
### R5-Specific Considerations

FHIR R5 introduced several structural changes and new resources that require special attention when anonymizing:

#### New Resources in R5
The R5 anonymization configuration includes support for these new resources:
- **Subscription** / **SubscriptionTopic**: Event notification subscriptions
- **ActorDefinition**: Definitions of actors in processes
- **Requirements**: Requirements for systems
- **ArtifactAssessment**: Assessment and rating of artifacts
- **RequestOrchestration**: Orchestration of multiple requests
- And many others (AdministrableProductDefinition, ArtifactAssessment, etc.)

#### Structural Changes in R5
Some resources have had fields moved or renamed:
- **Device**: `deviceName` → `name` 
- **PractitionerRole**: `availabilityExceptions` → `availability.notAvailableTime`
- **Appointment**: `comment` → `note`
- **CarePlan**: `activity.detail` → `activity.plannedActivityDetail`

Make sure your R5 configuration file includes rules for these updated paths.
