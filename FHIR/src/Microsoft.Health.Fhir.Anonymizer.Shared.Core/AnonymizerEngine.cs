...
namespace Microsoft.Health.Fhir.Anonymizer.Core
{
    public class AnonymizerEngine
    {
        ...
        public AnonymizerEngine(string configFilePath, IAnonymizerProcessorFactory customProcessorFactory = null) 
            : this(AnonymizerConfigurationManager.CreateFromConfigurationFile(configFilePath), customProcessorFactory)
        {
        }
        ...
        private void ValidateOutput(AnonymizerSettings settings, Resource resource)
        {
            if (settings != null && settings.ValidateOutput)
            {
                ...
                _validator.ValidateOutput(resource);
            }
        }
