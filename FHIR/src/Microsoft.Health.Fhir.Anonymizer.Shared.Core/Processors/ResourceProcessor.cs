    ...
    public class ResourceProcessor : IAnonymizerProcessor
    {
        ...
        public ProcessResult Process(ElementNode node, ProcessContext context = null, Dictionary<string, object> settings = null)
        {
            ...
            foreach (var rule in resourceRules)
            {
                ...
                {
                    throw new AnonymizerConfigurationException($"Anonymization method '{rule.Method}' is not supported. " +
                        $"Please register a custom processor for this method or use a built-in method.");
                }
        ...
        public void AddSecurityTag(ElementNode node, ProcessResult result)
        {
            ...
            if (result.IsRedacted && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.REDACT.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.REDACT);

            if (result.IsAbstracted && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.ABSTRED.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.ABSTRED);

            if (result.IsCryptoHashed && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.CRYTOHASH.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.CRYTOHASH);

            if (result.IsEncrypted && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.MASKED.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.MASKED);

            if (result.IsPerturbed && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.PERTURBED.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.PERTURBED);

            if (result.IsSubstituted && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.SUBSTITUTED.Code, System.StringComparison.InvariantCultureIgnoreCase)))
                meta.Security.Add(SecurityLabels.SUBSTITUTED);

            if (result.IsGeneralized && !meta.Security.Any(x =>
                string.Equals(x.Code, SecurityLabels.GENERALIZED.Code, System.StringComparison.InvariantCultureIgnoreCase)))
        ...
        private IEnumerable<ITypedElement> GetMatchNodes(AnonymizationFhirPathRule rule, ITypedElement node)
        {
            ...
            var descendantsOfTypeMatch = AnonymizationFhirPathRule.DescendantsOfTypeRuleRegex.Match(rule.Path);
            ...
            if (descendantsOfTypeMatch.Success)
                return GetMatchNodesFromLookUp(_typeToNodeLookUp, descendantsOfTypeMatch.Groups["type"].Value, string.Empty);
        ...
        private static IEnumerable<ITypedElement> GetMatchNodesFromLookUp(Dictionary<string, List<ITypedElement>> lookUp, string key, string expression)
        {
            ...
            {
                foreach (var node in lookUp[key])
        ...
        public ProcessResult ProcessNodeRecursive(ElementNode node, IAnonymizerProcessor processor, ProcessContext context, Dictionary<string, object> settings)
        {
            ...
            MarkNodeAndDescendantsAsVisited(node);
        ...
        /// <summary>
        /// Recursively marks a node and all its non-resource descendants as visited,
        /// preventing subsequent anonymization rules from processing them.
        /// </summary>
        private void MarkNodeAndDescendantsAsVisited(ElementNode node)
        {
            ...
            foreach (var child in node.Children())
            {
                if (child.IsFhirResource())
                    continue;
                MarkNodeAndDescendantsAsVisited((ElementNode)child);
            }
        }
    }
}
